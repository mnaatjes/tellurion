...
def test_run():
    ...
    #llm = GoogleGenAI(model="models/gemini-2.5-flash", api_key="")
    llm = GoogleGenAI()
    Settings.embed_model = embedding
    Settings.llm = llm

    LLAMA_CLONE = Path("tests/data/sources/tmp/llama_index_clone")
    LLAMA_SINK  = Path("tests/data/llama_index/")
    target_dir = "docs/src/content/docs/framework/"
    input_dir = Path(LLAMA_CLONE / target_dir)
    output_dir = Path(LLAMA_SINK / target_dir)
    ...
    print("LLAMA INDEX Vector Store COMPLETE!")